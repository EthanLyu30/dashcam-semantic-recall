...
- [x] 将完整 DVR-Semantic 原型资产接入仓库：`docs/prototype-source/`。
- [x] 保留原型中的 11 个核心页面、预览图、图片、Tailwind、Iconify、ECharts 等资源。
- [x] 编写原型搬运说明：`docs/prototype-migration.md`。
- [x] 编写设计系统：`DESIGN.md`，明确后续开发必须沿用原型视觉和导航。
- [x] 编写开发约定：`开发技巧.md`。
- [x] 编写无依赖原型运行入口：`python apps/desktop_client/run_prototype.py`。
- [x] 编写 Qt WebEngine 原型壳入口：`python apps/desktop_client/prototype_shell.py`。
- [x] 编写 Qt 客户端语义检索页脚手架，包含搜索、结果卡片、事件详情、时间轴和播放器占位。
...
- [ ] 如果课程要求必须是 Qt 窗口展示，安装 `PySide6` 与 `QtWebEngine` 后使用 `prototype_shell.py`。
